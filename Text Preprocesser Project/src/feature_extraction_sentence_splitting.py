...
def sentence_feature_extraction(text):
    ...
    for chr, i  in zip(text, range(len(text))):
        ...
        # apostroph_count += int(chr == '\'')

    return x, y

# def punctuation_feature_extraction(ch, next_ch, quote_count):
#     eos_markers = ':.!?'
#     x = [quote_count % 2, int(next_ch in eos_markers) - 0.5, int(next_ch == '\"') - 0.5,
#          int(next_ch.isdigit()) - 0.5]
#
#
